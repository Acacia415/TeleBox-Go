...
标题或搜索结果编号直接绑定，无需手动查看笔记 ID；也可以使用
`-ink new <简称> <标题>` 在 Telegram 中新建笔记并立即绑定。当前版本保留原消息的
段落及粗体、斜体、下划线、删除线、等宽、代码块、引用和链接；普通文字
中的 Markdown/HTML 标记会被转义。普通 Emoji 原样保留，自定义 Emoji
转换为对应的普通 Emoji，无法识别的占位符不会写入。插件不会意外生成
任务列表或折叠区块，也不会在 Telegram 中发送笔记地址。回复图片或普通
视频后可直接写入，每个附件不超过 25 MiB；GIF、Telegram 动图、贴纸及
其他文件不会上传。附件使用 Inkstone 自身的私有存储。完整配置方式见使用手册。
