...
图片和视频写入还要求 Inkstone 服务端支持 API 密钥附件上传。使用旧版
Inkstone 时，文字、新建和绑定功能不受影响，但媒体上传会提示先更新服务端。
...
也可以直接在 Telegram 中新建一篇空白笔记并绑定简称：

```text
-ink new hx 浩希
```

标题可以包含空格。简称已经存在时不会覆盖原绑定；如需更换，请先执行
`-ink unbind <简称>`。新建成功后只显示标题，不发送笔记地址。
...
回复一条 Telegram 文字、图片或普通视频消息后只发送 `-ink hx`，插件会
保存正文、媒体、正文外的链接，以及可以取得的发送者、时间、聊天名称和
消息链接。图片和视频可以没有文字。
...
图片和普通视频每个不能超过 25 MiB，附件保存在 Inkstone 自身的私有存储
中。图片会直接显示，视频会在预览中显示播放器；插件不会把附件地址发送到
Telegram，未登录 Inkstone 的访问者也不能读取。GIF、Telegram 动图、
静态/动态/视频贴纸、语音及其他文件不会上传，并会明确提示不支持。
